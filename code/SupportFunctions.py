...
def get_numbers(data_dir = '', simname = 'faccTHERMALzetaHG6.0RemMassFRYER2012SNDELAYED', 
    ...
    try:
        ...
        File        = h5.File(data_dir + simname +'/MainRun/COMPAS_Output.h5' ,'r')
    if verbose: print(File['BSE_System_Parameters'].keys())

    # Add relevant SYS keys to SYS_num table
    SYS_nums = Table()
    for key in ['SEED', 'Metallicity@ZAMS(1)', 'CE_Event_Counter', 'Merger', 'Optimistic_CE', 'Immediate_RLOF>CE']:
        SYS_nums[key] = File['BSE_System_Parameters'][key][()]
        
    # Check if mixture weight is there
    try:
        SYS_nums['mixture_weight'] = File['BSE_System_Parameters']['mixture_weight'][()]
    except:
        SYS_nums['mixture_weight'] = np.ones(len(SYS_nums))
