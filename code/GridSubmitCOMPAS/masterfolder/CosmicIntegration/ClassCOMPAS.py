...
class COMPASData(object):
    ...
    def setCOMPASDCOmask(
        ...
        print(f"sum(type_masks[BBH]) {sum(type_masks['BBH'])} sum(type_masks[BHNS]) {sum(type_masks['BHNS'])} sum(type_masks[BNS]) {sum(type_masks['BNS'])} sum(hubble_mask) {sum(hubble_mask)} ")
